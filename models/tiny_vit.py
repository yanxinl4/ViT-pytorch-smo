...
class TinyViTBlock(nn.Module):
    ...
    def forward(self, x):
        if self.smo and self.coef is not None:
            disturb1 = torch.randn_like(x) * self.coef
            x1 = self._process(x + disturb1)
            disturb2 = torch.randn_like(x) * self.coef
            x2 = self._process(x + disturb2)
            x = (x1 + x2) / 2
        else:
            x = self._process(x)
        return x
...
class TinyViT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000,
        ...
        self.smo = smo
        if self.smo:
            self.coef = nn.Parameter(torch.randn(1))
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer == 0:
                ...
                layer = BasicLayer(
                    ...
                    local_conv_size=local_conv_size,
                    smo=smo,
                    coef=self.coef if smo else None,
                    **kwargs)
